...
interface MisereGameProps {
  onBack: () => void;
}

type Player = "X" | "O" | null;

export default function MisereGame({ onBack }: MisereGameProps) {
  ...
  const checkWinner = (squares: Player[]) => {
    for (const line of winningLines) {
      ...
      if (
        ...
        // In Misère, the player who makes 3 in a row LOSES
        return squares[a];
      }
    }
    return null;
  ...
  const handleCellClick = (index: number) => {
    ...
    const loser = checkWinner(newBoard);
    if (loser) {
      // The player who made 3 in a row loses, so the other player wins
      const actualWinner = loser === "X" ? "O" : "X";
      setWinner(actualWinner);
      setGameEnded(true);
      if (actualWinner === "X") setScoreX(scoreX + 1);
      else setScoreO(scoreO + 1);
    } else if (newBoard.every((cell) => cell !== null)) {
      // If board is full and no one made 3 in a row, it's a draw
      setGameEnded(true);
    } else {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-2xl font-bold">Misère Tic-Tac-Toe</h2>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 mb-6">
        ...
        <p className="text-sm text-amber-700 dark:text-amber-400">
          The goal is to <strong>avoid</strong> making three in a row. The first
          player to get 3 in a row <strong>loses</strong>!
        </p>
      </div>
            ...
            {winner ? (
              <>
                ...
                <span className="text-lg font-semibold">
                  Player {winner} Wins! (Player {winner === "X" ? "O" : "X"}{" "}
                  made 3 in a row)
                </span>
              </>
            ) : (
              <span className="text-lg font-semibold">
                It&apos;s a Draw! No one made 3 in a row!
              </span>
            )}
          </div>
        ) : (
